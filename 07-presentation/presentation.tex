...
\begin{frame}{Results: Geographic Trends}
    \begin{center}
        \includegraphics[width=0.7\textwidth,height=0.6\textheight,keepaspectratio]{../04-analysis/outputs/figures/contract_ratio_map_2024.png}
    \end{center}
    \vspace{-0.2cm}
    \begin{itemize}
        \item \small \textbf{Hotspot States (2024):} Vermont (0.235), North Dakota (0.155), and Montana (0.137) show extreme reliance.
        \item \small \textbf{Regional Variation:} The Northeast and Midwest maintain the highest structural dependency.
    \end{itemize}
\end{frame}
...
\begin{frame}{Results: The Contagion Effect}
    \begin{columns}[c]
        \begin{column}{0.55\textwidth}
            \begin{center}
                \includegraphics[width=\textwidth,height=0.7\textheight,keepaspectratio]{../04-analysis/outputs/figures/county_contract_map_2024.png}
            \end{center}
        \end{column}
        \begin{column}{0.45\textwidth}
            \textbf{\footnotesize Proximity Regression:}
            \begin{itemize}
                \item \footnotesize \textbf{Neighbor Effect:} 0.3963 (p < 0.001)
                \item \footnotesize \textbf{Finding:} A 10\% increase in neighbors' usage predicts a ~4\% increase in own usage.
                \item \footnotesize \textbf{Contagion:} Strong evidence that staffing practices spread through local competition.
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}[shrink=5]{Discussion: Rural Vulnerability \& Agency Capture}
    \begin{itemize}
...
\begin{frame}[shrink=5]{Conclusion \& Policy Recommendations}
